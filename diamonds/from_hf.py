...
for obfuscate_suffix in suffixes:
    for seed in range(8):
        dataset_name = f"s{seed}{obfuscate_suffix}"
        folder = os.path.expanduser(f"{parent_folder}/{dataset_name}")
        dataset = load_dataset("redwoodresearch/diamonds", dataset_name)
