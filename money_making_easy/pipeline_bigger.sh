...
# export CUDA_VISIBLE_DEVICES=4,5,6,7 #,2,3,4,5,6,7
...
extra_args="--is_mpt"  # fix reinit as needed
# extra_args=""
...
train_and_evaluate() {
    ...
    if [ $DO_TRAIN -eq 1 ]; then
        ...
        accelerate launch --main_process_port 29632 --num_processes $num_processes $accelerate_args elk/func_correct/train_fsdp.py --model_name_or_path $model --output_dir $output_dir $train_args || exit 1
    fi
...
# train_and_evaluate $dirty_start_model_path "${model_folder}/${model_size}${extra_prefix}_model_dirty_jeft" "--excl_sensor_mask 0 1 1 1 1 $jeft_weights $args_nofreeze"
...
amnesic_base_args="--remove_after_layer -1 --remove_labels all_passes"
...
# train_and_evaluate "$find_mean_name/end_state/save_pretrained" "${model_folder}/${model_size}${extra_prefix}_jamnesic_clean_attn_probe" "--remove_after_layer -1 --remove_labels all_passes --lock_remover $clean_weights $args_freeze"
