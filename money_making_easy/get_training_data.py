...
with open("question_response_new_v4_3000_and_600_extra_tamp_for_val.jsonl", "r") as f:
...
is_val = [i < count_val or x.generated_item.tag == "extra_tamp_for_val" for i, x in enumerate(stories_shuf)]
...
with open("generate_v4_train.jsonl", "w") as f:
    for writ in train_stories:
        f.write(json_converter.dumps(writ) + "\n")

with open("generate_v4_val.jsonl", "w") as f:
...
def story_to_text(x: GeneratedStoryWithValues):
    return template.format(x.generated_item.money_item, x.generated_item.time_item, x.story)
...
print()
# print(template)
print(story_to_text(val_stories[0]))
...
def get_sensor_locs(input_ids: torch.Tensor):
    ...
    summed = torch.cumsum(torch.cat([eq_question_item, eq_question_item[:, -1:]], dim=-1), dim=-1)
    return (summed > 0) & ((input_ids == question_mark_tok[0]) | (input_ids == other_question_mark_tok[0]))
...
def batch_data_from_ntp(
    ...
    question_mark_locs = get_sensor_locs(input_ids)
    total_locs = torch.cumsum(question_mark_locs, dim=-1)
    total_overall = total_locs[:, -1]
    assert (total_overall == 3).all(), "can handle different cases, but assuming this is easiest"
    eqs = total_locs[:, :, None] == torch.arange(1, 4)[None, None]
    corresponding_locs = torch.where(
        eqs.any(dim=-2), torch.argmax(eqs.to(torch.uint8), dim=-2), input_ids.shape[-1] - 3
...
def is_clean(x: GeneratedStoryWithValues):
    return not any(w in x.story for w in bad_words)
...
base_dir = os.path.expanduser(f"~/money_making/v4/data_gpt_neo_x_tokenizer/")
print(f"{base_dir=}")
