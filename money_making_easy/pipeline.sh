...
# export CUDA_HOME=/home/ryan/miniconda3/envs/unity_env_fix/
...
small_model="EleutherAI/pythia-1.4b-deduped"
medium_model="cekal/mpt-7b-peft-compatible"
# run_dir="initial"
# run_dir="rerun"
run_dir="v3_new_code"
model_folder="$HOME/money_making/$run_dir/models/"
seq_len=1536
data_folder="$HOME/money_making/$run_dir/data_gpt_neo_x_tokenizer/"
eval_seed=$((seed + 1000))
epochs=6
...
# exit 1
# extra_args="--re_init_probes --is_mpt"  # fix reinit as needed
# extra_args="--is_mpt"
# extra_args="--re_init_probes"  # fix reinit as needed
extra_args=""

always_args="--num_warmup_steps 1 --mixed_precision fp16 --dataset_kind money_making --load_dir $data_folder --embeds_mode $embeds_mode --sensors_mode linear --dont_save_state --seed $seed --with_tracking --seq_len $seq_len $extra_args"

accelerate_args="--config_file $HOME/pythia_config.yaml"
# accelerate_args="--config_file $HOME/mpt_config.yaml"

# standard_args="--per_device_train_batch_size 8 --per_device_eval_batch_size 8 --num_train_epochs $epochs $always_args"
non_epoch_standard_args="--per_device_train_batch_size 8 --per_device_eval_batch_size 8 $always_args"
standard_args="--num_train_epochs $epochs $non_epoch_standard_args"
args_freeze="--learning_rate 3e-5 --freeze_layers all $standard_args"
...
dirty_weights_tok="--train_split train --ground_truth_weight 0"
...
evaluate() {
    ...
    local epoch=$2
    local device=$3
    local out_file="$model/$epoch/out_eval.txt"

    if [ $DO_EVAL -eq 1 ]; then
        echo "save to $out_file"

        DATASET_KIND=money_making CUDA_VISIBLE_DEVICES=${devices_arr[$device]} python elk/func_correct/eval_models.py run $model $epoch --load_dir $data_folder --split val | tee $out_file
    fi
}
...
train_and_evaluate() {
    ...
    if [ $DO_TRAIN -eq 1 ]; then
        ...
        accelerate launch --main_process_port 29674 --num_processes $num_processes $accelerate_args elk/func_correct/train_fsdp.py --model_name_or_path $model --output_dir $output_dir $train_args || exit 1
    fi
    evaluate $output_dir epoch_1 0 &
    evaluate $output_dir epoch_2 1 &
    wait
    evaluate $output_dir epoch_3 0 &
    evaluate $output_dir epoch_4 1 &
    wait
    ...
    # evaluate $output_dir epoch_3.0 2 &
    # evaluate $output_dir epoch_4.0 3 &
    # wait
...
dirty_start_model_path="${model_folder}/${model_size}${extra_prefix}_model_tok/epoch_3/save_pretrained/"

# train_and_evaluate $pretrain_model "${model_folder}/${model_size}${extra_prefix}_model" "$dirty_weights $args_nofreeze"
# train_and_evaluate $pretrain_model "${model_folder}/${model_size}${extra_prefix}_model_attn_tok" "$dirty_weights_tok $args_nofreeze"
train_and_evaluate $pretrain_model "${model_folder}/${model_size}${extra_prefix}_model_tok" "$dirty_weights_tok $args_nofreeze"
...
# # The GT probe model
# train_and_evaluate $dirty_start_model_path "${model_folder}/${model_size}${extra_prefix}_model_gt_probe_attn" "$gt_weights $args_freeze"
...
# train_and_evaluate $dirty_start_model_path "${model_folder}/${model_size}${extra_prefix}_model_clean_attn_probe" "$clean_weights $args_freeze"
...
# train_and_evaluate "${model_folder}/${model_size}${extra_prefix}_model_clean_attn_probe/epoch_3/save_pretrained" "${model_folder}/${model_size}${extra_prefix}_model_gt_probe_attn_freeze_from_clean" "$gt_weights $args_freeze --freeze_q_k_from_attn_probe"
...
# train_and_evaluate $dirty_start_model_path "${model_folder}/${model_size}${extra_prefix}_model_dirty_jeft_dp" "--excluded_set dirty-positive --excl_sensor_mask 0 1 1 1 1 $jeft_weights $args_nofreeze"
#
# train_and_evaluate $pretrain_model "${model_folder}/${model_size}${extra_prefix}_model_fresh_jeft" "--excl_sensor_mask 0 1 1 1 1 $jeft_weights $args_nofreeze"
...
# amnesic_base_args="--remove_after_layer last --remove_labels all_passes"

# find_mean_name="${model_folder}/${model_size}${extra_prefix}_model_find_mean"
# DO_EVAL=0 train_and_evaluate $dirty_start_model_path "$find_mean_name" "$amnesic_base_args --learning_rate 0.0 $non_epoch_standard_args --num_train_epochs 1 $dirty_weights --freeze_layers all --weight_decay 0.0"

# train_and_evaluate "$find_mean_name/end_state/save_pretrained" "${model_folder}/${model_size}${extra_prefix}_jamnesic_clean_attn_probe" "$amnesic_base_args --lock_remover $clean_weights $args_freeze"
# train_and_evaluate "$find_mean_name/end_state/save_pretrained" "${model_folder}/${model_size}${extra_prefix}_jamnesic_dirty_attn_probe" "$amnesic_base_args --lock_remover $dirty_weights $args_freeze"

# train_and_evaluate "$find_mean_name/end_state/save_pretrained" "${model_folder}/${model_size}${extra_prefix}_jamnesic_dirty_probe" "$amnesic_base_args --lock_remover $dirty_weights $args_freeze"
